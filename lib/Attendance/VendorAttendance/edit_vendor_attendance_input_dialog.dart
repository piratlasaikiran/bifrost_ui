...
class EditVendorAttendanceInputDialog extends StatefulWidget {
  final VendorAttendanceDTO vendorAttendanceDTO;
  const EditVendorAttendanceInputDialog({Key? key, required this.vendorAttendanceDTO}) : super(key: key);

  @override
  _EditVendorAttendanceInputDialog createState() =>
      _EditVendorAttendanceInputDialog();
}

class _EditVendorAttendanceInputDialog
    extends State<EditVendorAttendanceInputDialog> {
  ...
  void _initializeDataForEdit(){
    _selectedVendorId = widget.vendorAttendanceDTO.vendorId;
    _vendorIdController.text = widget.vendorAttendanceDTO.vendorId;
    commodityAttendance = widget.vendorAttendanceDTO.commodityAttendance.map((key, value) => MapEntry(key, value.toDouble()));
    _selectedSite = widget.vendorAttendanceDTO.site;
  ...
  Future<void> _enterVendorAttendance() async {
    if (_formKey.currentState!.validate()) {
      ...
      final result = await vendorActions.updateVendorAttendance(
          existingVendorAttendanceId: widget.vendorAttendanceDTO.vendorAttendanceId,
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: const Text('Vendor Attendance Edited Successfully.'),
                actions: [
                  TextButton(
                    onPressed: () {
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: const Text('Failed to edit vendor attendance.'),
                actions: [
                  TextButton(
                    onPressed: () {
                      Navigator.of(context).pop();
